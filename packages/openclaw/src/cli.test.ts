...
describe('patchConfig', () => {
  describe('add mode', () => {
    it('adds plugin to plugins.entries', () => {
      ...
      expect(entries['jeeves-meta-openclaw']).toEqual({ enabled: true });
    ...
    it('adds to plugins.allow when populated', () => {
      ...
      const plugins = config.plugins as Record<string, unknown>;
      expect(plugins.allow).toContain('jeeves-meta-openclaw');
    });
    ...
    it('adds to tools.allow when populated', () => {
      ...
      expect(tools.allow).toContain('jeeves-meta-openclaw');
    });

    it('does not duplicate if already present', () => {
      const config: Record<string, unknown> = {
        plugins: {
          allow: ['jeeves-meta-openclaw'],
          entries: {
            'jeeves-meta-openclaw': { enabled: true },
          },
        },
      };
      const msgs = patchConfig(config, 'add');
  ...
  describe('remove mode', () => {
    it('removes plugin from plugins.entries', () => {
      const config: Record<string, unknown> = {
        plugins: {
          entries: { 'jeeves-meta-openclaw': { enabled: true } },
        },
      };
      const msgs = patchConfig(config, 'remove');
      const plugins = config.plugins as Record<string, unknown>;
      const entries = plugins.entries as Record<string, unknown>;
      expect(entries).not.toHaveProperty('jeeves-meta-openclaw');
    ...
    it('removes from plugins.allow', () => {
      const config: Record<string, unknown> = {
        plugins: {
          allow: ['other', 'jeeves-meta-openclaw'],
          entries: {},
        },
      };
    ...
    it('removes from tools.allow', () => {
      const config: Record<string, unknown> = {
        tools: { allow: ['jeeves-meta-openclaw'] },
      };
      patchConfig(config, 'remove');
      const tools = config.tools as Record<string, unknown>;
